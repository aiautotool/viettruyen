...
export async function checkAudioGenerationAvailability(): Promise<boolean> {
  try {
    // Kiểm tra API mới vkct.synology.me
    try {
      const testText = "Test";
      ...
      // Thực hiện request POST với timeout ngắn
      ...
      console.warn("API vkct.synology.me không khả dụng:", vkctError.message);
    }
    
    // Fallback kiểm tra API aitoolseo.com
    try {
      const testText = "Test";
